...
contract LetsVaultFactory is Ownable, ReentrancyGuard, ILetsVaultFactory {
    ...
    constructor(address _vaultImplementation, address _feeRecipient, uint16 _protocolFeeBps) Ownable(msg.sender) {
        ...
        returns (address vault)
    {
        ...
        bytes32 salt = keccak256(abi.encode(msg.sender, asset, name, symbol));

        // Deploy vault using minimal proxy pattern
        vault = VAULT_IMPLEMENTATION.cloneDeterministic(salt);

        // Initialize vault
        ILetsVault(vault).initialize(asset, name, symbol, manager);
    ...
    function calculateVaultAddress(address asset, string memory name, string memory symbol, address deployer)
        ...
        returns (address)
    {
        bytes32 salt = keccak256(abi.encode(deployer, asset, name, symbol));
